...
def normalize_jleague_date(
    ...
    if full_year < season - 50:
        full_year += 100
    elif full_year > season + 50:
        full_year -= 100
...
def find_match(
    ...
    raise RuntimeError(
        ...
        f"page_id={page['id']}, "
        f"matches={unique_matches}"
    )


def load_candidate_seasons(
    match_date: str,
) -> tuple[int, ...]:
    """
    FootyStatsの暦年に加え、前年開始シーズンも候補にする。

    例:
        2026-12-13 → (2026, 2025)
        2027-04-24 → (2027, 2026)

    Jリーグ公式DBでは2026/27シーズンがseason=2026として
    保存されるため、2027年の試合では2026も検索対象にする。
    """
        ...
        for page in pages:
            ...
            if status == "not_found":
                ...
                print(
                    ...
                    f"candidate_seasons={candidate_seasons}"
                )
